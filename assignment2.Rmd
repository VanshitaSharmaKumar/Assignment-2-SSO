...
\]
```{r, fig.cap=c('2.3c Excel Solver'), out.width='60%', fig.show='hold', fig.align='center', fig.pos='H', echo=FALSE}
...
### 2.4a
This model minimizes total shipping cost from three sources to four destinations (D1–D4).
**Sets and data.**  
Sources \(S=\{\text{s1},\text{s2},\text{s3}\}\), Destinations \(D=\{\text{d1},\text{d2},\text{d3},\text{d4}\}\).  
Supply \(a_i\) for \(i\in S\), demand \(b_j\) for \(j\in D\), per-unit cost \(c_{ij}\) for \((i,j)\in S\times D\).

**Decision variables.**  
\(x_{ij}\ge 0\): quantity shipped from source \(i\) to destination \(j\).
...
**Concrete instance (your costs).**
\[
\begin{aligned}
\min\; & 10x_{\text{s1,d1}} + 20x_{\text{s1,d3}} + 11x_{\text{s1,d4}}
       + 12x_{\text{s2,d1}} + 7x_{\text{s2,d2}} + 9x_{\text{s2,d3}} + 20x_{\text{s2,d4}}
       + 14x_{\text{s3,d2}} + 16x_{\text{s3,d3}} + 18x_{\text{s3,d4}} \\[2pt]
\text{s.t.}\;
& x_{\text{s1,d1}} + x_{\text{s1,d2}} + x_{\text{s1,d3}} + x_{\text{s1,d4}} \le 20,\\
& x_{\text{s2,d1}} + x_{\text{s2,d2}} + x_{\text{s2,d3}} + x_{\text{s2,d4}} \le 25,\\
& x_{\text{s3,d1}} + x_{\text{s3,d2}} + x_{\text{s3,d3}} + x_{\text{s3,d4}} \le 15,\\
& x_{\text{s1,d1}} + x_{\text{s2,d1}} + x_{\text{s3,d1}} \ge 10,\\
& x_{\text{s1,d2}} + x_{\text{s2,d2}} + x_{\text{s3,d2}} \ge 15,\\
& x_{\text{s1,d3}} + x_{\text{s2,d3}} + x_{\text{s3,d3}} \ge 15,\\
& x_{\text{s1,d4}} + x_{\text{s2,d4}} + x_{\text{s3,d4}} \ge 20,\\
& x_{ij}\ge 0 \quad \forall i\in\{\text{s1,s2,s3}\},\; j\in\{\text{d1,d2,d3,d4}\}.
\end{aligned}
\]
...
\(y_{ij}\in\{0,1\}\): 1 if arc \((i,j)\) is used, else 0.

**Objective (per-unit cost + opening cost 100 if used).**
\[
\min_{x,y}\; \sum_{i\in S}\sum_{j\in D} \big(c_{ij}\,x_{ij} + 100\,y_{ij}\big).
\]

**Constraints (supply, demand, linking, domains).**
\[
\begin{aligned}
& \sum_{j\in D} x_{ij} \le a_i && \forall i\in S,\\
& \sum_{i\in S} x_{ij} \ge b_j && \forall j\in D,\\
& x_{ij} \le M_{ij}\,y_{ij} && \forall (i,j)\in S\times D \quad \text{(link: flow only if arc is opened)},\\
& x_{ij} \ge 0 && \forall (i,j)\in S\times D,\\
& y_{ij} \in \{0,1\} && \forall (i,j)\in S\times D.
\end{aligned}
\]
...
\[
\begin{aligned}
\min\; & (10x_{\text{s1,d1}}+100y_{\text{s1,d1}}) + (20x_{\text{s1,d3}}+100y_{\text{s1,d3}}) + (11x_{\text{s1,d4}}+100y_{\text{s1,d4}}) \\
& + (12x_{\text{s2,d1}}+100y_{\text{s2,d1}}) + (7x_{\text{s2,d2}}+100y_{\text{s2,d2}}) + (9x_{\text{s2,d3}}+100y_{\text{s2,d3}}) + (20x_{\text{s2,d4}}+100y_{\text{s2,d4}}) \\
& + (14x_{\text{s3,d2}}+100y_{\text{s3,d2}}) + (16x_{\text{s3,d3}}+100y_{\text{s3,d3}}) + (18x_{\text{s3,d4}}+100y_{\text{s3,d4}})
\end{aligned}
\]

with the same **supply/demand** constraints as in 2.4a, plus **linking constraints**
\(x_{ij}\le M_{ij}y_{ij}\) and domains \(x_{ij}\ge 0,\; y_{ij}\in\{0,1\}\).
